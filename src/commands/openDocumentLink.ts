...
export class OpenDocumentLinkCommand implements Command {
	...
	private async tryOpen(path: string, args: OpenDocumentLinkArgs) {
		...
		if (vscode.window.activeTextEditor && isGIFTFile(vscode.window.activeTextEditor.document) && vscode.window.activeTextEditor.document.uri.fsPath === resource.fsPath) {
